...
try {
  ...
  printResult(JSON.stringify(errObj));
}
    </pre>
...
try {
  ...
  jane1.data.name = "Janet Doe";
  await jane1.save();

  if (jane2.data.name == "Janet Doe") {
    printResult("fail - refreshed without asking");
    return;
  }

  await jane2.refresh();
  printResult(jane2.data.name == "Janet Doe" ? "pass" : "fail");
} catch (errObj) {
  printResult(errObj.textStatus);
}
    </pre>
...
try {
  ...
  console.log(jane.get(["name", "current", "salary", "start_date", "subordinates", "title"]));
  await jane.save();
...
try {
  ...
  entity.data.building = "J2";
  entity2.data.building = "B4";
  await Nymph.saveEntities(entities);
...
for (var i = 0; i < array.length; i++) {
  ...
  appendResult(i+": "+dashes.join("")+array[i].data.name+"\n");
}
appendResult("\n\n");
